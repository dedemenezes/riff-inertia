...
const searchText = computed({
  ...
  set: (value) => {
    filters.value.query = value
      ? { filter_display: value, filter_value: value, filter_label: simpleTranslation("Busca", "Search") }
      : null;
  },
});
...
const articleHref = (article) => `/${page.props.currentLocale}/noticias/${article.permalink}`;
...
<template>
  <Head>
    <title>{{ activePage }} - Festival do Rio</title>
  </Head>
  ...
  <Teleport to="body">
    <div v-show="isFilterOpen">
      ...
      <div
        class="fixed inset-y-0 right-0 z-50 bg-white flex flex-col w-full max-w-full h-[100dvh] shadow-lg md:max-w-[576px]"
      >
